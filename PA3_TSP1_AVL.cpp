    ...
    */
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            searchResult = edge[from].iterativeSearch(to);
            if (searchResult != NULL) //表示有路
            {
                if (maxPath[from] + searchResult->value > maxPath[to])
                {
                    maxPath[to] = maxPath[from] + searchResult->value;
                    if (maxPath[to] > max)
                    {
                        max = maxPath[to];
                    }
